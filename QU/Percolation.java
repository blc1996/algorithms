...
public class Percolation {
    ...
    private WeightedQuickUnionUF id1;
    ...
    public    void open(int row, int col){
        ...
        if(grid[row][col] == 0){
            ...
            id1.union(row*(dimension + 1) + col, (row - 1)*(dimension + 1) + col);
        }
        if(row < dimension){
            if(grid[row+1][col] == 1){
                id.union((row + 1)*(dimension + 1) + col, row*(dimension + 1) + col);
                id1.union((row + 1)*(dimension + 1) + col, row*(dimension + 1) + col);
            }
        }
        if(grid[row][col-1] == 1){
            id.union(row*(dimension + 1) + col, row*(dimension + 1) + col - 1);
            id1.union(row*(dimension + 1) + col, row*(dimension + 1) + col - 1);
        }
        if(col < dimension){
            if(grid[row][col+1] == 1){
                id.union(row*(dimension + 1) + col + 1, row*(dimension + 1) + col);
                id1.union(row*(dimension + 1) + col + 1, row*(dimension + 1) + col);
            }
        }
        }
    ...
    public boolean isFull(int row, int col){
        if(isOpen(row, col) && id1.connected(row*(dimension + 1) + col, 0)){
            return true;
        }else{
            return false;
        }
    ...
    public boolean percolates(){
        ...
        // }
        // return false;
        if(id.connected(dimension*(dimension + 1), 0))
            return true;
        else
            return false;
